...
theta0_string={'3\pi/4', '\pi/3', '\pi/2', '\pi', '\pi', '2\pi/3', '\pi/36', '\pi/2'};
T=40;
n=10;
format long
...
for j=1:length(s0)
    ...
    w=EulersolverRK(s0(j),theta0(j),T,N); %Run the eulersolver with RK method for this initial condition
...
figure('Position', [100, 100, 1024, 768]); % Adjust the last two values to set the width and height.
for i = 1:length(s0)
